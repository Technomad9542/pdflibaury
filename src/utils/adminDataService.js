...
export class AdminDataService {
  ...
  static async addNewPDF(pdfData) {
    try {
      const { category, subcategory, name, file_link, thumbnail, description, pages, size } = pdfData;
      ...
      const { data: resource, error: resourceError } = await supabase
        ...
        .insert({
          ...
          description: description || `Educational resource: ${name}`,
          pages: pages || Math.floor(Math.random() * 300) + 50,
          size: size || `${(Math.random() * 15 + 2).toFixed(1)} MB`,
          downloads: 0,
          rating: 0,
          // Tags will be auto-generated by the database trigger
        })
        .select()
        .single();
    ...
    try {
      ...
      const { data: newCategory, error: createError } = await supabase
        ...
        .insert({
          category,
          subcategory,
          description: `${category} - ${subcategory} resources`
        })
        .select()
        .single();
